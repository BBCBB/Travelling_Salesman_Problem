...
for iteration in range(1,maxiter+1):
    ...
    mo.append(mm)
    if iteration>lim:
        m=(o[iteration]-o[iteration-100])/100
        if min(mo[iteration-10000:iteration])-bestval>0:
            solution=bestsol.copy()
    else:
        m=-1
    if m<0.03 and m>=0 and mm>=0:
...
#----------------------------------------------------------------------------------------- 
    ...
    ok=0
    if iteration%maxthreshold==0:
        ok=1
    if ok==1 and mar>(sum(rec)/len(rec))*trs and indexmax!=nodeCount:
          ...
          tbc3opt.sort()
    elif ok==1 and mar>(sum(rec)/len(rec))*trs and indexmax==nodeCount:
